(() => {
  ...
  const DEFAULT_MODEL = { openrouter: 'openai/gpt-4o-mini', openai: 'gpt-4o-mini' };
  ...
  let settings = Object.assign({ provider: 'openrouter', apiKey: '', model: '' }, load(KEYS.cfg, {}));
...
${memory || '(aún no sabéis nada de ella, id conociéndola)'}`;
  ...
  function openSettings() {
    ...
    $('model').placeholder = DEFAULT_MODEL[settings.provider];
    $('memory').value = memory;
    dlg.returnValue = '';
    dlg.showModal();
  }
  dlg.addEventListener('close', () => {
    ...
    settings = { provider: $('provider').value, apiKey: $('apiKey').value.trim(), model: $('model').value.trim() };
    memory = $('memory').value.trim();
    save(KEYS.cfg, settings);
    save(KEYS.mem, memory);
  });
  $('provider').addEventListener('change', e => { $('model').placeholder = DEFAULT_MODEL[e.target.value]; });
  ...
  $('chatForm').addEventListener('submit', e => {
    ...
    if (!text) return;
    if (!settings.apiKey) { notice('Falta la API Key: ábrela con el botón 🔑.'); openSettings(); return; }
    $('input').value = '';
    addMsg('user', text);
    round();
  });
